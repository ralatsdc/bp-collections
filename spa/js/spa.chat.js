...
spa.chat = (function () {
    ...
    configMap = {
        main_html: String()
            ...
            + '        <input type="text"/>'
            + '        <input type="submit" style="dispay:none"/>'
    ...
    setJqueryMap = function () {
        ...
        jqueryMap = {
            ...
            $msg_in: $slider.find('.spa-chat-msg-in input[type=text]'),
     ...
     *   callback - optional callback to be run at the end of slider
     *              animation. The callback receives a jQuery
     *              collection representing the slider div as its
     *              single argument
     *
     * Action:
     *   Moves the slider into the requested position. If the
     *   requested position is the current position, it return true
    ...
    onSubmitMsg = function (event) {
        ...
        configMap.chat_model.send_msg(msg_test);
        jqueryMap.$input.focus();
        jqueryMap.$send.addClass('spa-x-select');
        setTimeout(
            function () {jqueryMap.$send.removeClass('spa-x-slelect');},
            250
        );
        return false;
    };
    ...
    onUpdatechat = function (event, msg_map) {
        ...
        chatee = configMap.chat_model.get_chatee() || {},
        sender = configMap.people_model.get_by_cid(send_id);
     ...
     *                     callback must return false if the requested
     *                     state cannot be met
     *   chat_model - the chat model object provides methods to
     *                interact with our instant messaging
     *   people_model - the people model object provides methods to
     *                  manage the list of people the model maintains
     *   slider_* - settings are optional scalars. See
     *              configMap.settable_map for a full list. Example:
     *              slider_open_em is the open height in ems
     *
     * Action:
    ...
    initModule = function ($append_target) {
        ...
        jqueryMap.$list+obx.bind('utap', onTapList);
     ...
     *
     * Purpose:
     *   -- Removes charSlider DOM element
    ...
    /* handleResize
     *
     * Purpose: Given a window resize event, adjust the presentation
     *   provided by the module if needed.
     *
     * Actions:
